...
import { useCreateVoucherMutation, useEditVoucherMutation, useFetchVoucherListingMutation, useGenerateCodesMutation, useGetGeneralLedgerQuery, useStatusUpdateMutation, useVoucherExportMutation, useVoucherGetByIdMutation, } from "../../store/musafirVoucherApi";
...
import { ApplicabilityOption, ConditionsListProps, Constraint, ConstraintsRulesType, Coupon, CustomSwitchProps, GetAllUserSegmentsResponse, GLCodesType, TableColumnProps, VoucherProps } from '../../utility/types/vouchers/voucher';
...
import { editSSValidationSchema, offerValidationSchemas } from "./VoucherValidationSchema";
dayjs.extend(utc);
                ...
                return (
                    ...
                    >
                        ...
                        {rule.RuleOptions?.filter((option) => !days.includes(option.MatchValue))
                            .map((option, index) => `${index === 0 ? '' : ', '}${capitalizeFirstLetter(formatValues(option.MatchValue))}`)}
                    </Typography>
                )
            })}
            {modelMode != 'view' && (
...
const VoucherNameField = ({ formik }): JSX.Element => {
    return (
        <TextField
            name="voucherName"
            type="text"
            ...
            onChange={(e) => formik.setFieldValue("voucherName", e.target.value)}
...
const VoucherPage: React.FC = (): JSX.Element => {
    ...
    const [applicabilityOptions, setApplicabilityOptions] = useState<ApplicabilityOption[]>([]);
    const [applicabilityOption, setApplicabilityOption] = useState<ApplicabilityOption>({} as ApplicabilityOption);
    ...
    const [getVoucherById] = useVoucherGetByIdMutation();
    const [fetchVoucherListing, { isLoading: islistLoading }] = useFetchVoucherListingMutation();
    const [voucherExport] = useVoucherExportMutation();
    const [editVoucherAPI] = useEditVoucherMutation();
    const [createVoucherAPI] = useCreateVoucherMutation();
    ...
    const isSameOfferId = selectedRows?.every(row => row?.VoucherId === selectedRows[0]?.VoucherId);
    const handleBulkEdit = () => {
        ...
        if (selectedRows.length === 1 || isSameOfferId) {
            getOfferViewById(selectedRows[0]?.VoucherId)
            setIsEditOffer(true);
        }
        else {
            formik.setFieldValue('voucherName', '(Multiple Files)')
    ...
    const [editData, setEditData] = useState<VoucherProps>({
        VoucherId: '',
        VoucherName: '',
        ShortDescription: '',
        LongDescription: '',
    ...
    const formik = useFormik({
        ...
        initialValues: {
            VoucherId: isEditOffer && editData?.VoucherId ? editData.VoucherId : "",
            voucherName: isEditOffer && editData?.VoucherName ? editData.VoucherName : "",
            userSegment: {
            ...
            userType: isEditOffer && editData?.UserType ? (editData.UserType === 'First time user' ? 'FirstTimeUser' : editData.UserType) : 'All',
            ...
            redemptionType: isEditOffer && editData?.CalculationDefinition?.RedemptionsType ? editData?.CalculationDefinition?.RedemptionsType : 'PARTIALLY'
        },
        ...
        onSubmit: async (values) => {
            if (isEditOffer || selectedRows?.length > 1) {
                await editOffer(editData?.VoucherId, values, selectedRow?.VoucherCouponId, selectedRows)
            } else {
    ...
    const offerExportApi = async () => {
        try {
            ...
            const response = await voucherExport(payload).unwrap();
    ...
    async function downloadCouponsAsCsv() {
        ...
        const fileName = `Voucher (${formattedDate}--${formattedTime}).csv`;
    ...
    const getOfferViewById = async (id: string) => {
        const payload = {
            ...
            Request: { "voucherId": id },
        };
        const response = await getVoucherById(payload);
        if (response?.data?.Context?.StatusCode === 1001) {
            setEditData(response?.data?.Response);
            setIsModelOpen(true);
        } else {
            customEnqueueSnackbar(response?.data?.Context?.Message ?? t("something_went_wrong"), 'error');
        }
    }
    ...
    const handleStatus = async (status: string) => {
        const offerIds = selectedRows?.map(row => row?.VoucherCouponId);
        let statusMessage;
        switch (status) {
            case 'INACTIVE':
                            ...
                            <CustomSwitch
                                ...
                                onChange={(e) => {
                                    ...
                                    if (row?.Status !== 'EXPIRED') {
                                        handleToggleStatus(row?.VoucherCouponId, row?.Status);
                                    }
                                }}
        ...
        try {
            ...
            const payload = {
                ...
                Request: {
                    ...
                    ...(values?.userType.replace(/\s+/g, "_") === "ONCE_PER_CUSTOMER" ||
                        values?.usageType === "SINGLE"
                        ? {}
                        : { Frequency: values?.frequency === '' ? 1 : values?.frequency }),
                    UserType:
                        values?.userType == "First time user"
                            ? "FirstTimeUser"
                            : values?.userType.replace(/\s+/g, "_"),
                    GLCodeId: values?.GLCodeId,
                    Validity: {
                        StartDateTime: formatDateTimeToUTC(
                            values?.startDate ? dayjs(values?.startDate) : null,
                            values?.startTime
                        ),
                        EndDateTime: formatDateTimeToUTC(
                    ...
                    CalculationDefinition: {
                        ...
                        DiscountValue:
                            values?.markdownType === "FIXEDAMOUNT"
                                ...
                                : values?.percentage,
                        ...(values?.markdownType != "FIXEDAMOUNT" && {
                            ApplicableOn: values?.applicableOn?.replace(/\s+/g, "_"),
                        }),
                        ...(values?.markdownType === "FIXEDAMOUNT"
                            ? {}
                            : { MaxLimit: values?.discountValue }),
                    },
                    Constraints: values?.constraints,
                    ApplicableDevices: values?.applicableDevices?.map((i) =>
                        i.toUpperCase()
                    ),
                    PaymentModes: values?.paymentModes?.map((i) =>
                        i.replace(/\s+/g, "").toUpperCase()
                    ),
                    CouponCodes: values?.couponCodes,
                },
            };
            const data = await createVoucherAPI(payload).unwrap();
    ...
    const editOffer = async (id: string, values: typeof formik.values, VoucherId, selectedRows) => {
        ...
        const payload = {
            ...
            "Request": {
                "Coupons": selectedRows.length > 1 ? selectedRows.map(row => ({
                    ...
                    VoucherCouponIds: [row.VoucherCouponId],
                })) : [{
                    "VoucherId": id,
                    "VoucherCouponIds": selectedRows.length == 1 ? [selectedRows[0]?.VoucherCouponId] : [VoucherId]
                }],
                "OrgEntityId": "*",
                "VoucherName": (selectedRows.length > 1 && values?.voucherName === '(Multiple Files)') ? '' : values?.voucherName,
                "ShortDescription": values?.shortDescription,
                "LongDescription": values?.longDescription,
                "TermsAndConditions": values?.termsAndConditions,
                "UserSegmentId": values?.userSegment.Id,
        ...
        try {
            const data = await editVoucherAPI(payload).unwrap();
    ...
    const fetchCouponData = async (pageNumber: number = 1, pageSize: number = 5, search: string = "") => {
        try {
            ...
            try {
                await fetchVoucherListing(payload).unwrap()
                    .then(response => {
        ...
        onRowCheckboxChange: (row: Coupon, type: "all" | "single") => {
            if (type === "all") {
                ...
                setCoupons((prevCoupons) => prevCoupons?.map((coupon) =>
                    coupon.VoucherCouponId === row.VoucherCouponId ? { ...coupon, isSelected: !coupon.isSelected } : coupon
                ));
                if (row.isSelected) {
                    setSelectedRows((prev) => Array.isArray(prev) ? prev?.filter(r => r.VoucherCouponId !== row.VoucherCouponId) : []);
                } else {
    ...
    const viewOffer = (): JSX.Element => {
        return (
            <Box sx={{ width: { xs: '100vw', md: '70vw' }, margin: "auto", marginTop: !isMobileView ? '20px' : '0px', px: 4, pb: !isMobileView ? 4 : 0, pt: !isMobileView ? 1 : 0, }}>
                ...
                <Box sx={{
                    ...
                    <Box sx={{
                        ...
                        <Typography sx={{
                            ...
                            {editData?.VoucherCoupons?.[0]?.VoucherCouponName ?? ''}
                        </Typography>
                        <Box
                            sx={{
                                textTransform: "none",
                    ...
                    <Box sx={{ display: 'flex', justifyContent: isMobileView ? 'space-between' : undefined, width: "100%", flexDirection: isMobileView ? 'column' : 'row' }}>
                        ...
                        <Box sx={{
                            ...
                            <Typography sx={{
                                ...
                                color: theme?.palette?.customColors?.black[1],
                                fontWeight: "500"
                            }} >{editData?.VoucherName}</Typography>
                        </Box>
                    </Box>
                    {/*Applicability*/}
                    <Box sx={{ display: 'flex', justifyContent: !isMobileView ? 'space-between' : undefined, width: "100%", flexDirection: isMobileView ? 'column' : 'row' }}>
                        <Box sx={{
                            ...
                            <Typography sx={{
                                ...
                                color: theme?.palette?.customColors?.grey[8],
                                fontWeight: "400"
                            }} >Sets the user segments, which can avail this voucher</Typography>
                        </Box>
                        <Box sx={{
                            display: "flex",
                            alignItems: "flex-start",
                    ...
                    <Box sx={{ display: 'flex', justifyContent: isMobileView ? 'space-between' : undefined, width: "100%", flexDirection: isMobileView ? 'column' : 'row' }}>
                        <Box sx={{
                            ...
                            <Typography sx={{
                                ...
                                color: theme?.palette?.customColors?.grey[8],
                                fontWeight: "400"
                            }} >Add Short Description, Description and T&C for your voucher</Typography>
                        </Box>
                        <Box
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', width: "100%" }}>
                        <Box sx={{
                            ...
                            <Typography sx={{
                                ...
                                fontWeight: "500",
                                color: theme?.palette?.customColors?.black[1],
                            }} >Voucher Constraints</Typography>
                            <Typography sx={{
                                fontSize: "11px",
                                color: theme?.palette?.customColors?.grey[8],
                                fontWeight: "400"
                            }} >Configures voucher to make it redeemable only in certain</Typography>
                        </Box>
                        <Box sx={{
                            display: "flex",
                            alignItems: "flex-start",
                    ...
                    <Box sx={{ display: 'flex', justifyContent: isMobileView ? 'space-between' : undefined, width: "100%", flexDirection: isMobileView ? 'column' : 'row' }}>
                        <Box sx={{
                            ...
                            <Typography sx={{
                                ...
                                color: theme?.palette?.customColors?.grey[8],
                                fontWeight: "400"
                            }} >Sets whether the voucher will expire in one use or multiple</Typography>
                        </Box>
                        <Box sx={{
                            display: "flex",
                            alignItems: "flex-start",
                    ...
                    <Box sx={{ display: 'flex', justifyContent: isMobileView ? 'space-between' : undefined, width: "100%", flexDirection: isMobileView ? 'column' : 'row' }}>
                        <Box sx={{
                            ...
                            <Typography sx={{
                                ...
                                fontWeight: "500",
                                color: theme?.palette?.customColors?.black[1],
                            }} >Voucher Validity</Typography>
                            <Typography sx={{
                                fontSize: "11px",
                                color: theme?.palette?.customColors?.grey[8],
                                fontWeight: "400"
                            }} >Sets the validity criteria for this voucher</Typography>
                        </Box>
                        <Box sx={{
                            display: "flex",
                            alignItems: "flex-start",
                    ...
                    <Box sx={{ display: 'flex', justifyContent: !isMobileView ? 'space-between' : undefined, width: "100%", flexDirection: isMobileView ? 'column' : 'row' }}>
                        <Box sx={{
                            ...
                            <Typography sx={{
                                ...
                                color: theme?.palette?.customColors?.grey[8],
                                fontWeight: "400"
                            }} >Sets the devices, on which this voucher is redeemable</Typography>
                        </Box>
                        <Box sx={{
                            display: "flex",
                            alignItems: "flex-start",
                    ...
                    <Box sx={{ display: 'flex', justifyContent: isMobileView ? 'space-between' : undefined, width: "100%", flexDirection: isMobileView ? 'column' : 'row' }}>
                        <Box sx={{
                            ...
                            <Typography sx={{
                                ...
                                fontWeight: "500",
                                color: theme?.palette?.customColors?.black[1],
                            }} >Markdown Value</Typography>
                            <Typography sx={{
                                fontSize: "11px",
                                color: theme?.palette?.customColors?.grey[8],
                                fontWeight: "400"
                            }} >Configures the price markdown for your voucher</Typography>
                        </Box>
                        <Box sx={{
                            display: "flex",
                            alignItems: "flex-start",
                            ...
                            <Typography sx={{
                                ...
                                color: theme?.palette?.customColors?.black[1],
                                fontWeight: "500"
                            }} >{currency?.IsoCode3}{" "}{editData?.CalculationDefinition?.DiscountValue}</Typography>
                        </Box>
                    </Box>
                    <Box sx={{ display: 'flex', ml: 2, gap: 2, flexDirection: "column" }}>
                        <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                            <Typography sx={{ fontSize: "12px", fontWeight: "500", fontStyle: "italic" }}>Partially Redeemable*</Typography>
                            <Typography sx={{ fontSize: "12px", fontWeight: "400" }}>{editData?.CalculationDefinition?.RedemptionsType ?? "Yes"}</Typography>
                        </Box>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: isMobileView ? 'space-between' : undefined, width: "100%", flexDirection: isMobileView ? 'column' : 'row' }}>
                        <Box sx={{
                            ...
                            <Typography sx={{
                                ...
                                voucher for redemption</Typography>
                        </Box>
                        <Box sx={{
                            display: "flex",
                            alignItems: "flex-start",
                ...
                <CustomDrawer isOpen={isDescModalOpen} anchor={isMobileView ? 'bottom' : 'right'}
                    sx={{
                        ...
                        <Box sx={{ display: "flex", flexDirection: "column", gap: 5 }}>
                                ...
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', width: "100%" }}>
                                    <Box sx={{
                                        ...
                                        <Typography sx={{
                                            ...
                                            color: theme?.palette?.customColors?.grey[8],
                                            fontWeight: "400"
                                        }} >Displayed as the banner of your voucher</Typography>
                                    </Box>
                                </Box>
                                <Box sx={{
                                    display: "flex",
                            ...
                            <Box sx={{ display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', width: "100%" }}>
                                    <Box sx={{
                                        ...
                                        <Typography sx={{
                                            ...
                                            color: theme?.palette?.customColors?.grey[8],
                                            fontWeight: "400"
                                        }} >Provides more details about the voucher</Typography>
                                ...
                                <Box sx={{
                                    ...
                                    <Typography dangerouslySetInnerHTML={{ __html: editData?.LongDescription }} /></Box>
                            </Box>
    ...
    const applicability = (): JSX.Element => {
        return (<>
            {modelMode != 'editSingleScreen' && (
                <>
                    <Box sx={{ display: "flex", alignItems: "center", justifyContent: "flex-end", mb: !isMobileView ? "25px" : "0px", ml: "7rem", mr: isMobileView ? undefined : "-4px" }}>
                        <ClearIcon onClick={() => setIsModelOpen((prev) => !prev)} />
                    </Box>
                    <VoucherNameField formik={formik} />
                    <Divider sx={{ color: theme?.palette?.customColors?.lightGray[12], mt: 2, mb: 4 }} />
                </>
            )}
            <Box sx={{ display: 'flex', justifyContent: !isMobileView ? 'space-between' : undefined, width: "100%", flexDirection: isMobileView ? 'column' : 'row', gap: '34px', }}>
                <Box sx={{
                    display: "flex",
                    flexDirection: "column",
                    alignItems: "flex-start",
                ...
                <Box sx={{ mb: 4 }}>
                    <Box sx={{
                        ...
                        <Box sx={{
                            ...
                            }} >Describe your voucher*</Typography>
                        </Box>
                    </Box>
                </Box>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', width: "100%" }}>
                    <Box sx={{
                        ...
                        <Typography sx={{
                            ...
                            color: theme?.palette?.customColors?.grey[8],
                            fontWeight: "400"
                        }} >Configures voucher to make it redeemable only in certain conditions</Typography>
                    </Box>
                    <Box sx={{
                        display: "flex",
                        alignItems: "flex-start",
                ...
                {!(modelMode === 'view' || modelMode === 'editSingleScreen') && formik.values?.constraints?.length > 0 && <Box sx={{
                    ...
                    <Box sx={{
                        ...
                        <Typography sx={{
                            ...
                            color: theme?.palette?.customColors?.grey[8],
                            fontWeight: "400"
                        }} >Generate or use your own codes to use for redemption
                        </Typography>
                    </Box>
                </Box>}
                {
    ...
    const BpCheckedBigIcon = styled(BpBigIcon)(({ theme }) => ({
        ...
        'input:hover ~ &': {
            backgroundColor: theme?.palette?.customColors?.lightBlue[9], // Darker blue for hover
        },
    }));    // Checked icon with your custom blue styling

    const BpCheckedIcon = styled(BpIcon)({
        backgroundColor: theme?.palette?.customColors?.blue[22], // Your preferred blue
        backgroundImage: 'linear-gradient(180deg,hsla(0,0%,100%,.1),hsla(0,0%,100%,0))',
        '&::before': {
            display: 'block',
            width: 16,
                    ...
                    <Box sx={{
                        ...
                        <Box sx={{
                            ...
                            <Typography sx={{
                                ...
                                'Sets the usability configuration for this voucher' :
                                'Sets the usage and qualifying criteria for this voucher'}</Typography>
                        </Box>
                        <Box>
                            {selectedRows?.length > 1 ? <Typography sx={{
                                ...
                                color: theme?.palette?.customColors?.black[1],
                                fontWeight: "500"
                            }} >{capitalizeFirstLetter(formik.values?.usageType)}</Typography> : (
                                <ToggleButtonGroup
                    ...
                    && (<Box sx={{
                        ...
                        <Box>
                            ...
                            <Typography sx={{
                                ...
                                color: theme?.palette?.customColors?.grey[8],
                                fontWeight: "400"
                            }} >Sets the type of users that can avail this voucher</Typography>
                        </Box>
                                ...
                                <FormControlLabel
                                    ...
                                    control={
                                        <Radio
                                            ...
                                            color="default"
                                            checkedIcon={<BpCheckedIcon />}
                                            checked={formik.values?.userType === 'First time user' || formik.values?.userType === 'First time user'}
                                            size="small"
                                            icon={<BpIcon />}
                                        />
                                    }
                ...
                {formik.values?.usageType === 'MULTIPLE' && (<Box sx={{ my: 4, pl: !isMobileView ? '20px' : '0px', display: "flex", flexDirection: "row", justifyContent: "space-between" }}>
                    <Box sx={{
                        ...
                        <Typography sx={{
                            ...
                            color: theme?.palette?.customColors?.grey[8],
                            fontWeight: "400"
                        }} >Sets the usage frequency for users and ways in which it can be used
                        </Typography>
                    </Box>
                    <Box sx={{
                        display: "flex",
                    ...
                    <>  <Box sx={{
                        ...
                        <Box sx={{
                            ...
                            <Typography sx={{
                                ...
                                color: theme?.palette?.customColors?.grey[8],
                                fontWeight: "400"
                            }} >Sets the devices on which this voucher is redeemable
                            </Typography>
                        </Box>
                        <Box sx={{
                            display: "flex",
                ...
                {modelMode != 'editSingleScreen' && (
                    <>
                        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: !isMobileView ? "25px" : "0px", }}>
                            {!isMobileView && (
                                <>
                                    <IconButton sx={{ p: 0 }} onClick={() => setStep((prev) => prev - 1)} >
                                        <ArrowBackIcon />
                                    </IconButton>
                    ...
                    <Box sx={{
                        ...
                        <Box sx={{
                            ...
                            <Box sx={{
                                ...
                                <Typography sx={{
                                    ...
                                    Configures the price markdown for your voucher
                                </Typography>
                            </Box>

                            <FormControl variant="outlined">
                                <FormHelperText id="outlined-weight-helper-text" sx={{
                                    textAlign: 'left', mx: '0px'
                        ...
                        >
                            ...
                            <Box sx={{ maxWidth: '300px' }}>
                                <Typography sx={{ fontSize: '12px', fontWeight: '500', fontStyle: modelMode === 'editSingleScreen' ? 'italic' : "unset" }}>
                                    Partially Redeemable*
                                </Typography>
                            </Box>
                            <Box
                            ...
                            >
                                <ToggleButtonGroup
                                    ...
                                    onChange={(_event, newValue) => {
                                        if (newValue !== null) {
                                            formik.setFieldValue('redemptionType', newValue);
                                        }
                                    }}
                                    sx={{
                                        '& .MuiToggleButton-root': {
                                        ...
                                        '& .Mui-selected': {
                                            backgroundColor: `${theme?.palette?.customColors?.lightBlue[3]} !important`,
                                            borderRadius: '6px',
                                        },
                                    }}
                                >
                                    <ToggleButton value="PARTIALLY">Yes</ToggleButton>
                                    <ToggleButton value="COMPLETELY">No</ToggleButton>
                                </ToggleButtonGroup>
                            </Box>
                        </Box>
                        {/* Payment Mode  */}
                        <Box sx={{ display: 'flex', justifyContent: !isMobileView ? 'space-between' : undefined, width: '100%', my: 2, flexDirection: isMobileView ? 'column' : 'row', gap: isMobileView ? 2 : 0, }}>
                            <Box sx={{
                                ...
                                <Typography sx={{
                                    ...
                                    Sets the payment modes, which can be used along with this voucher for redemption
                                </Typography>
                            </Box>
    ...
    const editSingleScreenOffer = () => {
        return (
            <Box sx={{ width: { xs: '100vw', md: '70vw' }, margin: "auto", marginTop: !isMobileView ? '20px' : '0px', px: 4 }}>
                <Box sx={{ display: "flex", alignItems: "center", justifyContent: "flex-end", mr: "-5px" }}>
                    <ClearIcon onClick={() => {
                        formik.setFieldValue("userSegment", applicabilityOptions[0]);
                        setIsModelOpen(false);
                ...
                <Typography sx={{ fontSize: "22px", fontWeight: "600" }}>{(selectedRows?.length > 1 && isSameOfferId) ? '(Multiple Files)' : editData?.VoucherCoupons?.[0]?.VoucherCouponName ?? ''}</Typography>
                <Divider sx={{ color: theme?.palette?.customColors?.lightGray[12], mt: 2, mb: 4 }} />
                {/* Voucher Name */}
                <Box sx={{ display: 'flex', justifyContent: !isMobileView ? 'space-between' : undefined, width: "100%", mb: 2, flexDirection: isMobileView ? 'column' : 'row' }}>
                    ...
                    <Box sx={{
                        ...
                        <TextField
                            ...
                            onBlur={formik.handleBlur}
                            error={formik.touched?.voucherName && Boolean(formik.errors?.voucherName)}
                            helperText={formik.touched?.voucherName && formik.errors?.voucherName as string}
                            slotProps={{
                                input: {
                                    endAdornment: (
                                        <InputAdornment position="end" sx={{ color: theme?.palette?.customColors?.lightWhite[8], cursor: "pointer" }}>
                                            <ClearIcon
                                                onClick={() => formik.setFieldValue("voucherName", "")}
                                                sx={{ fontSize: 20 }}
                                            />
                                        </InputAdornment>
                                    ),
                        ...
                        <ToggleButtonGroup
                            ...
                            onChange={(e, v) => {
                                ...
                                if (v === 'SINGLE') {
                                    ...
                                    formik.setFieldValue("userType", "ONCE_PER_CUSTOMER")
                                }
                            }}
                            onBlur={formik.handleBlur}
                            sx={{
                ...
                <Box sx={{ mb: 3, mt: 2, display: 'flex', flexDirection: !isMobileView ? 'row' : 'column', justifyContent: !isMobileView ? 'space-between' : undefined, width: "100%" }}>
                    ...
                    <Box sx={{
                        ...
                        {formik.values?.usageType === 'SINGLE' && (<Box sx={{ display: 'flex', justifyContent: isMobileView ? 'flex-start' : undefined, width: isMobileView ? '100%' : undefined, pl: 0, }}>
                            <RadioGroup
                                value={formik.values?.userType}
                                onChange={(e) => formik.setFieldValue('userType', e.target.value)}
                                onBlur={formik.handleBlur}
                                ...
                                <FormControlLabel
                                    ...
                                    control={
                                        <Radio
                                            ...
                                            color="default"
                                            checkedIcon={<BpCheckedIcon />}
                                            checked={formik.values?.userType === 'FirstTimeUser'}
                                            size="small"
                                            icon={<BpIcon />}
                                        />
                                    }
                                    ...
                                    <>  To {
                                        formik.values?.endTime
                                            ...
                                            : (editData?.Validity?.EndDateTime
                                                ? formatDateTime(editData?.Validity?.EndDateTime)?.time || 'NA'
                                                : 'NA')
                                    }</>)}
                            </Typography>
                        </Box>
    ...
    const openNewOffer = () => {
        ...
        formik.setFieldValue("voucherName", "");
        setModelMode('create')
        setIsModelOpen(true);
        setIsEditOffer(false);
        setSelectedRows([])
        ...
        try {
            ...
            await generateOfferCodes(payload).unwrap().then((response) => {
                if (response.Context.StatusCode === 1001) {
                    ...
                    customEnqueueSnackbar(response?.Context?.Message ? response?.Context?.Message : t('workflow_updated_successfully'), 'success', 'left');
            ...
            <Box sx={{ width: isMobileView ? '98%' : "100%", mb: '2rem', mt: "2rem" }}>
                ...
                <Box sx={{
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', my: 2, height: isMobileView ? "30px" : "36px" }}>
                        <TextField
                            placeholder={isMobileView ? 'Search by Voucher Name, Voucher code...' : 'Search Vouchers or Search by Date of Creation by From: (Date) To: (Date)'}
                            variant="outlined"
                            value={offerSearch}
                            onChange={(e) => {
                ...
                {
                    (coupons.length === 0 && !islistLoading) ? (<Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", marginTop: '10%', }}>
                        <Typography sx={{ fontSize: 24, color: "gray" }}>No data Found</Typography></Box>) : (<CustomTable {...tableProps} />)
                }
            </Box>
                            ...
                            <ClickAwayListener onClickAway={handleMenuClose}>
                                ...
                                >
                                    <MenuItem onClick={() => {
                                        ...
                                        setModelMode('view');
                                        setIsModelOpen(true);
                                        getOfferViewById(selectedRow?.VoucherId);
                                        setIsEditOffer(true);
                                    }}>
                                        <ListItemIcon>
                                    ...
                                    {selectedRow?.Status != 'EXPIRED' && (
                                        <>
                                            ...
                                            {selectedRow?.Status === 'ACTIVE' && (
                                                ...
                                                <MenuItem onClick={() => {
                                                    handleMenuClose();
                                                    handleToggleStatus(selectedRow?.VoucherCouponId, selectedRow?.Status);
                                                }}>
                                                    <ListItemIcon>
                                            ...
                                            {selectedRow?.Status === 'INACTIVE' && (
                                                ...
                                                <MenuItem onClick={() => {
                                                    ...
                                                    <ListItemIcon>
                                                        <img src={Activate} alt='Edit icon' width={18} height={18} />
                                                    </ListItemIcon>
                                    ...
                                    <MenuItem onClick={() => {
                                        handleToggleStatus(selectedRow?.VoucherCouponId, 'ARCHIVED')
                                        handleMenuClose();
                                    }}>
                                        <ListItemIcon>
