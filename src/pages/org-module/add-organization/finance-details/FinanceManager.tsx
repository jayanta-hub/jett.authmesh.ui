...
import {
  FinanceManagerProps, FormField,
  FormValues
} from "../../../../utility/types/organization/financeManager";
import { usePostFinanceMangerDetailsMutation } from "../../../../store/musafirOrgApi";
import LoadingScreen from "../../../../components/core-module/loading-screen/LoadingScreen";
import { clearData } from "../../../../store/slice/OrgSlice";
...
const FinanceManager: React.FC<FinanceManagerProps> = ({ activeStep, setActiveStep }): JSX.Element => {
  const { t } = useTranslation();
  const theme = useTheme();
  const isSmallScreen = useMediaQuery(theme.breakpoints.down("sm"));
  const [postOrg, { isLoading }] = usePostFinanceMangerDetailsMutation();
  ...
  const { values, errors, handleSubmit,
    ...
    handleBlur, touched } = useFormik<FormValues>({
      ...
      onSubmit: (values: FormValues) => {
        const submitFinanceManagerDetails = async () => { 
          const payload = {
            entityId: entityId,
            name: values.name,
            email: values.email,
          ...
          try {
            ...
            if (response?.status?.statusCode === "SC00001") {
              enqueueSnackbar(response?.status?.statusDesc || 'Finance manager details added successfully.', {
                variant: 'success',
                anchorOrigin: {
                  vertical: 'bottom',
                  horizontal: 'right',
