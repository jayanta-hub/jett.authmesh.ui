...
import TravelManager from "./TravelManagerDetails";

import '@testing-library/jest-dom';
import { usePostTravelMangerDetailsMutation } from "../../../../store/musafirOrgApi"; // Mock this API call
...
jest.mock("../../../../store/musafirOrgApi", () => ({
  usePostTravelMangerDetailsMutation: jest.fn(),
}));
...
describe("TravelManager Component", () => {
  let store: any;
  let postTravelManagerMock: jest.Mock;

  beforeEach(() => {
    store = mockStore(initialState);
    postTravelManagerMock = jest.fn();
    (usePostTravelMangerDetailsMutation as jest.Mock).mockReturnValue([postTravelManagerMock, { isLoading: false }]);
  });

  test("renders the form with all fields", () => {
    render(
      <Provider store={store}>
        <TravelManager activeStep={0} setActiveStep={jest.fn()} />
      </Provider>
    );
    ...
    expect(screen.getByText("travel_manager_details")).toBeInTheDocument();
  ...
  test("form submits successfully", async () => {
    ...
    await userEvent.type(screen.getByLabelText(/job_title/i), "Travel Manager");
    ...
    await waitFor(() => expect(postTravelManagerMock).toHaveBeenCalledTimes(1));
    expect(postTravelManagerMock).toHaveBeenCalledWith({
      entityId: "12345",
      organizationId: 0,
      name: "John Doe",
  ...
  test("form shows loading state while submitting", async () => {
    postTravelManagerMock.mockReturnValueOnce([jest.fn(), { isLoading: true }]);
  ...
  test("handles error in form submission", async () => {
    postTravelManagerMock.mockRejectedValueOnce(new Error("Network error"));
